...
export default function IssuingAuth({ permit }: any) {
  ...
  async function rejectPermit() {
    const [_, err] = await rejectApi.makeRequest({ id: +permit.id });
    if (err) {
      return toast({
        variant: "error",
    ...
    <div className="app-section">
      {permit.completionIssuringAuthSupStatus === "IN_PROGRESS" ? (
        ...
        <Accordion show title="Approval Details">
          ...
          <AccordionItem
            title="Date"
            value={dayjs(permit.completionIssuringAuthSupStatusDate).format(
              "MMM DD, YYYY HH:mm A"
            )}
          />
      ...
      <Modal
        ...
        align="center"
      >
        <ModalHeader>Issuing Auth. Supervisor</ModalHeader>
        <ModalBody>
          {/* prettier-ignore */}
