...
import { completePermit, suspendPermit } from "../../../../assets/api/permit";
...
any) {
  ...
  const hazards = HAZARDS.filter((h) => permit[h.value]);
  ...
  async function onSubmit(data) {
    ...
    if (modals.suspension) toggle("suspension");
  ...
  async function completePermitToWork() {
    const [res, err] = await completionApi.makeRequest({ id: permit.id });
    if (err) {
      return toast({
        variant: "error",
    ...
    if (modals.completion) toggle("completion");
    toast({
      variant: "success",
      message: res.message,
    });
  ...
  return (
    <div className="app-section">
      <Accordion
        ...
        title={
          <>
            <span>Section A - </span>Work Description
          </>
        }
      >
        <AccordionItem
          title="Work Type"
          value={capitalize(permit.type?.replace("_", " ").toLowerCase())}
        />
        <AccordionItem
        ...
        />
        <AccordionItem title="Type of Process" value={permit.processType} />
        <AccordionItem
          title="Contractor"
          value={
            permit.contractor
              ? `${permit.contractor?.firstname} ${permit.contractor?.lastname}`
              : "--"
          }
        />
        <AccordionItem title="Contact Tel. No." value={permit.contactTel} />
        <AccordionItem title="Contact Phone No." value={permit.contactPhone} />
        <AccordionItem title="No. of Personnel for Activity" value="18" />
        <AccordionItem
        ...
        <AccordionItem
          ...
          value={
            hazards.map((h) => capitalize(h.text.toLowerCase())).join(", ") ||
            "--"
          }
        />
        <AccordionItem
          title="Permit Valid From (Date & Time)"
          value={dayjs(permit.validityStartDate).format(
            "DD, MMM, YYYY, HH:mm A"
          )}
        />
        <AccordionItem
        ...
        <AccordionItem
          ...
          value={permit.previousPtwRef?.join(", ")}
        />
      </Accordion>
      ...
      <Accordion
        title={
          <>
            <span>Section B - </span>Job & Work Hazards
          </>
        }
      >
        <AccordionItem
        ...
        />
        <AccordionItem title="Zone" value={permit.zone} />
        <AccordionItem title="Equipment Tag" value={permit.equipmentTag} />
        ...
        {permit.type === "HOT_WORK" && (
          <>
            ...
            />
            <AccordionItem title="P&IDs / Sketch" value={permit.sketchDocNo} />
            <AccordionItem
              title="Lifting Plan"
            ...
            />
            <AccordionItem title="Others" value={permit.otherDocNo} />
            <AccordionItemTitle>Identification Of Hazards</AccordionItemTitle>
            {HAZARDS.map((hazard) => (
              <AccordionItem
                title={hazard.text}
      ...
      <Accordion
        title={
          <>
            <span>Section C - </span>Document Uploads
          </>
        }
      >
        <AccordionItem
      ...
      <Accordion
        title={
          <>
            <span>Section D - </span>Additional Notes
          </>
        }
      >
        <AccordionItem title={permit.additionalNotes ?? "---"} value="" />
      </Accordion>
...

      {
        ...
        permit?.status === "APPROVED" ? (
          <div>
            <div className="app-ptw__completion-cta">
              <h1>Work Completion</h1>
              <p>
                The work is completed, all tools an equipment are reinstated in
                their initial position, and worksite is left clean and safe.
              </p>
              <Button
                variant="success"
                dimension="md"
            ...
            <div className="app-ptw__completion-cta suspension">
              <h1>Work Suspension</h1>
              <p>
                The work is completed, all tools an equipment are reinstated in
                their initial position, and worksite is left clean and safe.
              </p>
      ...
      <Modal
        ...
        align="center"
      >
        <ModalHeader>Work Completion</ModalHeader>
        <ModalBody>
          {/* prettier-ignore */}
          ...
          <h4 className="app-modal__content__title">Work Completion</h4>
          <p className="app-modal__content__desc">
            By proceeding, you can confirm the work is completed, all tools an
            equipment are reinstated in their initial position, and worksite is
            left clean and safe.
          </p>

          <div className="app-modal__content__footer">
            <Button variant="secondary" onClick={() => toggle("completion")}>
              Cancel
            </Button>
            <Button
              variant="success"
      ...
      >
        ...
        <ModalBody>
          <form onSubmit={handleSubmit}>
            <div className="app-ptw-reject-modal">
              <p className="app-modal__content__desc">
                By rejecting this permit, it means you have verified all the
                details and document provided and they do not match the required
                standards.
                <br />
                <br />
                Kindly state the reason for rejecting this permit below;
              </p>
