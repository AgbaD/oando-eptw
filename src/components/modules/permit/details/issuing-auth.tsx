...
export default function IssuingAuth({ permit }: any) {
  // const rejectApi = useRequest();
  // const approveApi = useRequest();
  const { getFieldProps, handleSubmit } = useForm({
    initialValues: { notes: "", permitId: +permit.id },
  ...
  //   const [_, err] = await approveApi.makeRequest({ notes: "", permitId: +permit.id })
  //   if (err) {
  //     return toast({
  //       variant: "error",
  ...
  return (
    <div className="app-section">
      {isHotWork && isApproved && (
        ...
        >
          <AccordionItemTitle>MEASURES ON EQUIPMENT / LINES</AccordionItemTitle>
          {permit.measuresOnEquipment?.map((t) => (
            <AccordionItem title={t} />
          ))}

          <AccordionItemTitle>PRECAUTION</AccordionItemTitle>
          {permit.precautions?.map((t) => (
            <AccordionItem title={t} />
          ))}

          <AccordionItemTitle>FIREFIGHTING PRECAUTION</AccordionItemTitle>
          {permit.firefightingPrecautions?.map((t) => (
            <AccordionItem title={t} />
          ))}
          ...
          {permit.protectiveEquipment?.map((t) => (
            <AccordionItem title={t} />
          ))}
          ...
          <AccordionItem title="Entry Certificate" value={permit.entryCertNo} />
          ...
          <AccordionItem title="MEWP Cert." value={permit.mewpCertNo} />
          ...
          <AccordionItem title="Lift Cert." value={permit.liftCertNo} />
      ...
      {permit.issuringAuthorityStatus === "IN_PROGRESS" ? (
        <div className="app-ptw__details__module">
          <h4>Approval Section</h4>
          <p>
            To approve or reject this permit, ensure you have properly gone
            through the PTW Details and confirmed all details and documents
            provided. Kindly fill in addition notes below to add more details
            (Additional notes must be filled if you are rejecting this permit,
            to state the reason for rejection).
          </p>
          ...
          <div className="app-ptw__details__module__btn-footer">
            <Button
              ...
              onClick={() =>
                isHotWork
                  ? route(`/permit-management/ptw/approve/${permit.id}`)
                  : toggle("confirmApproval")
              }
            >
              Approve Permit
            </Button>
        ...
        <Accordion show title="Approval Details">
          ...
          <AccordionItem
            title="Date"
            value={dayjs(permit.issuringAuthorityStatusDate).format(
              "MMM DD, YYYY HH:mm A"
            )}
          />
          <AccordionItem title="Approved by:" value="ROBERT FOX" />
          <AccordionItem
      ...
      <Modal
        ...
        align="center"
      >
        <ModalHeader>Issuing Auth</ModalHeader>
        <ModalBody>
          {/* prettier-ignore */}
