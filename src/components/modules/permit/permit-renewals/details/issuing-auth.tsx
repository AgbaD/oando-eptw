...
export default function IssuingAuth({ permit }: any) {
  ...
  const {
    ...
    initialValues: { notes: "", renewalId: +permit.id },
  ...
  if (permit.safetyOfficerStatus !== "APPROVED") {
    return (
  ...
  return (
    <div className="app-section">
      {permit.issuringAuthoritySupStatus === "IN_PROGRESS" ? (
        ...
        <Accordion show title="Approval Details">
          ...
          <AccordionItem
            title="Date"
            value={dayjs(permit.issuringAuthoritySupStatusDate).format(
              "MMM DD, YYYY HH:mm A"
            )}
          />
          <AccordionItem
